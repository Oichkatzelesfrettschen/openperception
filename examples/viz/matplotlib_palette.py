...
TOKENS = json.loads(
    Path(__file__).resolve().parents[2].joinpath("tokens/color-tokens.json").read_text()
)
...
def apply_brand_cycler(variant: str = "default"):
    ...
    ls = [
        (0, tuple(d if d != 0 else 1 for d in dash)) if dash != [0, 0] else "solid"
        for dash in dashes
    ]

    plt.rcParams["axes.prop_cycle"] = (
        cycler(color=colors) + cycler(marker=mm) + cycler(dashes=ls)
    )
