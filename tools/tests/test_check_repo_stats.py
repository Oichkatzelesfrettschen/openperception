...
def seed_repo(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (tmp_path / "papers" / "downloads" / "topic" / "paper.pdf").write_text(
        "pdf", encoding="utf-8"
    )
...
def test_generate_repo_stats_counts_expected_surfaces(tmp_path: Path) -> None:
    (tmp_path / "papers" / "downloads" / "algorithms").mkdir(
        parents=True, exist_ok=True
    )
    (tmp_path / "papers" / "downloads" / "algorithms" / "one.pdf").write_text(
        "pdf", encoding="utf-8"
    )
    ...
    (tmp_path / "research" / "topic" / "REPORT.md").write_text(
        "# report\n", encoding="utf-8"
    )
    (tmp_path / "research" / "topic" / "primary_source_notes.md").write_text(
        "# notes\n", encoding="utf-8"
    )
    (tmp_path / "docs" / "external_sources").mkdir(parents=True, exist_ok=True)
    (tmp_path / "docs" / "external_sources" / "topic_source_cache.md").write_text(
        "# cache\n", encoding="utf-8"
    )
    (tmp_path / "datasets" / "source_assets" / "demo").mkdir(
        parents=True, exist_ok=True
    )
    (tmp_path / "datasets" / "source_assets" / "demo" / "asset.pdf").write_text(
