...
def check_c_library_binary() -> bool:
    binary = REPO_ROOT / "algorithms" / "libDaltonLens" / "build" / "tests" / "test_simulation"
    if not binary.exists():
        print("[SKIP] libDaltonLens C test binary: not built -- run `make build-c` first")
...
def check_validator_imports() -> bool:
    ...
    try:
        ...
        ok = len(specs) >= 6
        return _check("validator_registry (GATE specs)", ok, f"{len(specs)} gates registered")
    except Exception as exc:
