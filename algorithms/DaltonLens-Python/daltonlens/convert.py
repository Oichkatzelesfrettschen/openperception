...
class ImageEncoding(Enum):
    SRGB = 0
    LINEAR_RGB = (
        1  # assume the image is already in linearRGB and don't apply any transform
    )
...
def gamma22_from_linearRGB(im):
    ...
    return np.power(np.clip(im, 0.0, 1.0), 1.0 / 2.2)
...
def desaturate_linearRGB_to_fit_in_gamut(im):
    ...
    return np.clip(im - min_val[:, np.newaxis], 0.0, 1.0)
...
class LMSModel_Vischeck_GIMP(LMSModel):
    ...
    vischeck_LMS_from_linearRGB = np.array(
        [
    ...
    vischeck_linearRGB_from_LMS = np.array(
        [
    ...
    def __init__(self):
        super().__init__(
            self.XYZ_from_linearRGB, self.LMS_from_XYZ, usesJuddVosXYZ=False
        )
...
class LMSModel_sRGB_SmithPokorny75(LMSModel):
    ...
    def __init__(self, ignoreJuddVosCorrection=False):
        ...
        super().__init__(
            self.XYZ_from_linearRGB,
            self.LMS_from_XYZ,
            usesJuddVosXYZ=(not ignoreJuddVosCorrection),
        )
...
class LMSModel_sRGB_HuntPointerEstevez(LMSModel):
    ...
    # Hunt-Pointer-Estevez is older and supposedly worse than e.g. Bradford
    # for chromatic adaptation, but it is better to simulate the human
    # cone responses, which is what we need for CVD simulation.
    #
    # This transform is the version normalized for D65, which means that
    # L=M=S=1 for the XYZ=D65 white point. The normalization does not affect
    # CVD simulation results because the Brettel/Vienot algorithms project
    # colors onto a plane in LMS space, and this projection is invariant
    # to uniform scaling of the LMS axes. Different normalizations produce
    # different absolute LMS values but identical relative positions, so
    # the dichromatic projection yields the same final RGB output.
