...
def name_of_deficiency(d: Deficiency):
    if d == Deficiency.PROTAN:
        return "protan"
    if d == Deficiency.DEUTAN:
        return "deutan"
    if d == Deficiency.TRITAN:
        return "tritan"
    if d == Deficiency.ACHROMAT:
        return "achromat"
    if d == Deficiency.BCM:
        return "bcm"
...
class Simulator(ABC):
    ...
    def simulate_cvd(self, image_srgb_uint8, deficiency: Deficiency, severity: float):
        ...
        im_cvd_linear_rgb = self._simulate_cvd_linear_rgb(
    ...
    def _simulate_cvd_linear_rgb(
        self, image_linear_rgb_float32, deficiency: Deficiency, severity: float
    ):
...
class DichromacySimulator(Simulator):
    ...
    def _simulate_cvd_linear_rgb(
        ...
        im_dichromacy = self._simulate_dichromacy_linear_rgb(
            image_linear_rgb_float32, deficiency
        )
        if severity < 0.99999:
            return im_dichromacy * severity + image_linear_rgb_float32 * (
                1.0 - severity
            )
        else:
    ...
    @abstractmethod
    def _simulate_dichromacy_linear_rgb(
        self, image_linear_rgb_float32, deficiency: Deficiency, severity: float
    ):
        pass
...
def plane_projection_matrix(plane_normal, deficiency: Deficiency):
    ...
    if deficiency == Deficiency.PROTAN:
        return np.array([[0.0, -n[1] / n[0], -n[2] / n[0]], [0, 1, 0], [0, 0, 1]])
    # Projection along the M axis
    if deficiency == Deficiency.DEUTAN:
        return np.array([[1, 0, 0], [-n[0] / n[1], 0, -n[2] / n[1]], [0, 0, 1]])
    # Projection along the S axis
    if deficiency == Deficiency.TRITAN:
        return np.array([[1, 0, 0], [0, 1, 0], [-n[0] / n[2], -n[1] / n[2], 0]])
    return None
...
class Simulator_Vienot1999(DichromacySimulator):
    ...
    def __init__(
        self, color_model: convert.LMSModel = convert.LMSModel_sRGB_SmithPokorny75()
    ):
        super().__init__()
        self.color_model = color_model

    def _simulate_dichromacy_linear_rgb(
        self, image_linear_rgb_float32, deficiency: Deficiency
    ):
        ...
        if deficiency == Deficiency.PROTAN or deficiency == Deficiency.DEUTAN:
            ...
            v_red = self.color_model.LMS_from_linearRGB @ np.array(
                [1.0, 0.0, 0.0]
            )  # - lms_black which is ommitted since it's zero
            v_cyan = self.color_model.LMS_from_linearRGB @ np.array(
                [0.0, 1.0, 1.0]
            )  # - lms_black which is ommitted since it's zero
        ...
        self.cvd_linear_rgb = (
            self.color_model.linearRGB_from_LMS
            @ self.lms_projection_matrix
            @ self.color_model.LMS_from_linearRGB
        )

        if self.dumpPrecomputedValues:
            print(
                array_to_C_decl(
                    f"vienot_{name_of_deficiency(deficiency)}_rgbCvd_from_rgb",
                    self.cvd_linear_rgb,
                )
            )
...
class Simulator_Brettel1997(DichromacySimulator):
    ...
    def _simulate_dichromacy_linear_rgb(
        ...
        if self.use_white_as_neutral:
            ...
            xyz_E = [0.8, 0.8, 0.8]
            self.color_model.linearRGB_from_XYZ @ xyz_E
        ...
        def compute_matrices(lms_on_wing1, lms_on_wing2):
            ...
            n_sep_plane = np.cross(
                lms_neutral, lms_confusion_axis(deficiency)
            )  # separation plane going through the diagonal
        ...
        im_lms = convert.apply_color_matrix(
            image_linear_rgb_float32, self.color_model.LMS_from_linearRGB
        )
        ...
        im_linear_rgb = convert.apply_color_matrix(
            im_H, self.color_model.linearRGB_from_LMS
        )
...
class Simulator_Vischeck(Simulator_Brettel1997):
    ...
    def __init__(self):
        super().__init__(
            convert.LMSModel_Vischeck_GIMP(),
...
class Simulator_Machado2009(Simulator):
    ...
    def _simulate_cvd_linear_rgb(
        ...
        severity_lower = math.floor(severity * 10.0)
...
class Simulator_BCM(Simulator):
    ...
    def _simulate_cvd_linear_rgb(
        ...
        min_val = np.minimum(im_cvd[..., 0], np.minimum(im_cvd[..., 1], im_cvd[..., 2]))
...
class Simulator_AutoSelect(Simulator):
    ...
    def _simulate_cvd_linear_rgb(
        ...
        if deficiency == Deficiency.ACHROMAT:
            ...
            model = _lms_cache.setdefault(
                "SmithPokorny75", convert.LMSModel_sRGB_SmithPokorny75()
            )
        ...
        return simulator._simulate_cvd_linear_rgb(
            image_linear_rgb_float32, deficiency, severity
        )
