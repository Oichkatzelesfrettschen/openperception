...
def ishihara_image(fg_color, bg_color, mask):
    ...
    try:
        ...
        sys.stderr.write(
            "OpenCV is required for ishihara_image: `pip install opencv-python'\n"
        )
        sys.exit(1)
...
def ishihara_plate_dichromacy(
    ...
    for uv_plane in lms_seeds:
        ...
        utils.normalized(p2 - p1)
        np.linalg.norm(p2 - p1)
        srgb_1, srgb_2 = convert.sRGB_from_linearRGB(
            convert.apply_color_matrix(np.array([p1, p2]), lms_model.linearRGB_from_LMS)
        )
        ...
        mask = (
            ...
            if n > 0
            else np.zeros((256, 256, 1))
        )
        if mask is None:
            print(
                f"ERROR: could not load the mask from {mask_file}, does the file exist?"
            )
            return None
...
def simulator_ishihara_plate(
    ...
    try:
        ...
        sys.stderr.write(
            "colour is required for simulator_ishihara_plate: `pip install colour-science'\n"
        )
        return None
    ...
    for r, g, b in itertools.product(per_axis_seed, per_axis_seed, per_axis_seed):
        rgb_refs.append(
            grid[int(r * num_steps), int(g * num_steps), int(b * num_steps)]
        )
    rgb_refs = np.array(rgb_refs)
    rgb_refs_cvd = simulator._simulate_cvd_linear_rgb(
        rgb_refs.reshape(1, -1, 3), deficiency, severity=severity
    ).reshape(-1, 3)
    ...
    def linearRGB_to_Lab(im_rgb):
        return colour.XYZ_to_Lab(colour.sRGB_to_XYZ(im_rgb, apply_cctf_decoding=False))

    # Manage a subset of colors in the grid.
    ColorSubset = namedtuple(
        "ColorSubset", ["orig_rgb", "orig_lab", "cvd_rgb", "cvd_lab"]
    )
    ...
    for rgb_ref, rgb_ref_cvd in zip(rgb_refs, rgb_refs_cvd, strict=False):
        ...
        grid[indices]
        grid_cvd[indices]
        current_set = current_set._replace(
            orig_lab=linearRGB_to_Lab(current_set.orig_rgb)
        )
        current_set = current_set._replace(
            cvd_lab=linearRGB_to_Lab(current_set.cvd_rgb)
        )
