...
try:
    ...
    sys.stderr.write(
        "Geometry3D is required to import this module: `pip install Geometry3D'\n"
    )
    raise e


def geo3dPoint_from_np(p):
    return geo3d.Point(p[0], p[1], p[2])


def geo3dVector_from_np(p):
    return geo3d.Vector(p[0], p[1], p[2])


def np_from_geo3dPoint(p):
    return np.array([p.x, p.y, p.z])


def lms_confusion_segment(
    lms_color, color_model: convert.LMSModel, deficiency: simulate.Deficiency
):
    ...
    lms_K = color_model.LMS_from_linearRGB @ np.array([0.0, 0.0, 0.0])
    lms_R = color_model.LMS_from_linearRGB @ np.array([1.0, 0.0, 0.0])
    lms_G = color_model.LMS_from_linearRGB @ np.array([0.0, 1.0, 0.0])
    lms_B = color_model.LMS_from_linearRGB @ np.array([0.0, 0.0, 1.0])

    # The parallelogram is defined by the origin (black) and the
    # 3 vectors.
    KBMRGCWY_geometry = geo3d.Parallelepiped(
        geo3dPoint_from_np(lms_K),
    ...
    line = geo3d.Line(
        geo3dPoint_from_np(lms_color), geo3dPoint_from_np(lms_color + confusion_axis)
    )
    ...
    if segment is None:
        print(
            "ERROR: lms_confusion_segment: the provided color is not in the RGB gamut."
        )
        return None
